(() => {
    ...
    function tabs() {
        ...
        function initTabs(tabsBlock) {
            ...
            tabsTitles.forEach((tabTitle => {
                tabTitle.addEventListener("mouseenter", setTabsAction);
            }));
    ...
    function slicerEffect({swiper, extendParams, on, emit}) {
        extendParams({
            slicerEffect: {
                split: 5
            }
        ...
        const createClones = () => {
            swiper.slides.forEach((slide => {
                if (slide.querySelector(".slide-news__image-clones")) return;
                const originalImg = slide.querySelector(".slide-news__image");
                if (!originalImg) return;
                const clonesWrapper = document.createElement("div");
                clonesWrapper.className = "slide-news__image-clones";
                for (let i = 0; i < swiper.params.slicerEffect.split; i++) {
                    const cloneBox = document.createElement("div");
                    cloneBox.className = "slide-news__image-clone";
                    const img = originalImg.cloneNode(true);
                    img.style.marginLeft = `-${100 * i}%`;
                    img.style.width = `${document.documentElement.clientWidth}px`;
                    cloneBox.style.left = `${100 / swiper.params.slicerEffect.split * i}%`;
                    cloneBox.appendChild(img);
                    clonesWrapper.appendChild(cloneBox);
                }
                slide.insertBefore(clonesWrapper, originalImg.nextSibling);
            }));
        };
        const updateClonesSize = () => {
            swiper.slides.forEach((slide => {
                document.querySelectorAll(".slide-news__image-clones").forEach((clonesWrapper => {
                    clonesWrapper.style.width = `${swiper.width}px`;
                }));
                slide.querySelectorAll(".slide-news__image-clone img").forEach((img => {
        ...
        const applyTransforms = () => {
            ...
            swiper.slides.forEach(((slide, i) => {
                slide.style.transform = `translate${axis}(-${100 * i}%)`;
                const progress = slide.progress;
                const clones = slide.querySelectorAll(".slide-news__image-clone");
                clones.forEach((clone => {
                    clone.style.transform = `translate${axis}(${100 * -progress}%)`;
                }));
                const content = slide.querySelector(".slide-news__info");
                if (content) content.style.transform = `translate${axis}(${swiper.size * -progress * 1.2}px)`;
            }));
        };
        const applyTransitions = duration => {
            swiper.slides.forEach((slide => {
                const clones = slide.querySelectorAll(".slide-news__image-clone");
                const content = slide.querySelector(".slide-news__info");
                ...
                clones.forEach(((clone, index) => {
                    ...
                    if (duration === 0) clone.style.transitionDuration = isLoopFixing ? "0ms" : `${swiper.params.speed + swiper.params.speed / (total - 1) * (total - index - 1)}ms`; else clone.style.transitionDuration = `${duration + duration / (total - 1) * (total - index - 1)}ms`;
                }));
            }));
        };
        on("beforeInit", (() => {
            ...
            Object.assign(swiper.originalParams, swiper.params);
        }));
        on("init", (() => {
        ...
        on("resize", (() => {
            if (swiper.params.effect === "slicer") updateClonesSize();
        }));
        on("slideChange", (() => {
            if (swiper.virtual && swiper.params.virtual.enabled) requestAnimationFrame((() => {
                createClones();
                updateClonesSize();
            }));
        }));
        on("beforeLoopFix", (() => isLoopFixing = true));
        on("loopFix", (() => isLoopFixing = false));
        on("setTranslate", (() => {
            if (swiper.params.effect === "slicer") applyTransforms();
        }));
        on("setTransition", ((_, duration) => {
            if (swiper.params.effect === "slicer") applyTransitions(duration);
        }));
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new Swiper(".news__slider", {
            modules: [ Navigation, Pagination, Autoplay, slicerEffect ],
            effect: "slicer",
            slicerEffect: {
                split: 5
            },
    ...
    function headerScroll() {
        ...
        let timer;
        if (main) main.addEventListener("windowScroll", (function(e) {
            if (main.getBoundingClientRect().top <= 0) {
                clearTimeout(timer);
                const headerShowTimer = header.dataset.scrollShow ? header.dataset.scrollShow : 100;
                const scrollTop = window.scrollY;
                const startPoint = header.dataset.scroll ? -parseInt(header.dataset.scroll) : main.getBoundingClientRect().top;
                if (main.getBoundingClientRect().top <= startPoint) {
    ...
    setTimeout((() => {
        if (addWindowScrollEvent) {
            ...
            window.addEventListener("scroll", (function(e) {
                ...
                main.dispatchEvent(windowScroll);
            }));
        }
    }), 0);
    class DynamicAdapt {
        ...
        init() {
            ...
            this.nodes.forEach((node => {
                ...
                object.nextElement = object.element.nextElementSibling;
                this.objects.push(object);
            }));
            this.arraySort(this.objects);
            this.mediaQueries = this.objects.map((({breakpoint}) => `(${this.type}-width: ${breakpoint}px),${breakpoint}`)).filter(((item, index, self) => self.indexOf(item) === index));
            this.mediaQueries.forEach((media => {
                const mediaSplit = media.split(",");
                const matchMedia = window.matchMedia(mediaSplit[0]);
                const mediaBreakpoint = mediaSplit[1];
                const оbjectsFilter = this.objects.filter((({breakpoint}) => breakpoint === mediaBreakpoint));
        ...
        mediaHandler(matchMedia, objects) {
            console.log(`mediaHandler triggered, match: ${matchMedia.matches}`);
            if (matchMedia.matches) objects.forEach((object => {
                this.moveTo(object.place, object.element, object.destination);
            })); else objects.forEach((({parent, element, nextElement}) => {
                if (element.classList.contains(this.daClassname)) this.moveBack(parent, element, nextElement);
            }));
        }
        ...
        moveBack(parent, element, nextElement) {
            element.classList.remove(this.daClassname);
            if (nextElement && nextElement.parentNode === parent) parent.insertBefore(element, nextElement); else parent.appendChild(element);
        }
    ...
    function updateParallax(scrollY) {
        ...
        if (fullscreen && fullscreen.classList.contains("_watcher-view")) !previewImg.classList.contains("visible") ? previewImg.classList.add("visible") : null;
        if (previewImg) previewImg.style.transform = `translateY(-${offset}px)`;
    }
    function raf(time) {
    ...
    requestAnimationFrame(raf);
    function openHeroMenu(e) {
        if (e.target.tagName === "BUTTON") {
            let button = e.target;
            let wrapper = button.closest(".article-hero__wrapper");
            wrapper.classList.toggle("open");
        }
    }
    const heroCont = document.querySelector(".hero__container");
    if (heroCont) heroCont.addEventListener("click", openHeroMenu);
    document.addEventListener("DOMContentLoaded", (() => {
        const container = document.querySelector(".cont");
        const elements = [ ...document.querySelectorAll(".el") ];
        const closeButtons = [ ...document.querySelectorAll(".el__close-btn") ];
        function layoutGrid() {
            ...
            elements.forEach(((el, i) => {
                ...
                const y = row === 0 ? `0%` : `calc(${100 * row}% + ${spacingYpx * row}px)`;
                el.style.transform = `translate3d(${x}, ${y}, 0)`;
                el.style.transformOrigin = `calc(${x} + 50%) calc(${y} + 50%)`;
                const inner = el.querySelector(".el__inner");
                if (inner) inner.style.transitionDelay = `${.1 * i}s`;
            }));
        }
        function updateGridState() {
            ...
            elements.forEach(((el, i) => {
                ...
                const x = `${spacingX * col}%`;
                const y = row === 0 ? `0%` : `calc(${100 * row}% + ${spacingYpx * row}px)`;
                if (container.classList.contains("s--el-active") && !el.classList.contains("s--active")) {
                    el.style.transform = `scale(0.5) translate3d(${x}, ${y}, 0)`;
                    el.style.opacity = "0";
                    el.style.transition = "transform 0.95s, opacity 0.95s";
                } else {
                    el.style.transform = `translate3d(${x}, ${y}, 0)`;
                    el.style.opacity = "";
                    el.style.transition = "";
        ...
        const observer = new IntersectionObserver(((entries, observer) => {
            entries.forEach((entry => {
                ...
                } else container.classList.add("s--inactive");
            }));
        }), {
            threshold: .3
        });
