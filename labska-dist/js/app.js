...
***************************************************************************** */
        ...
        (() => {
            const mapNodes = document.querySelectorAll(".js-map");
            ...
            loader.load().then(google => {
                mapNodes.forEach($map => {
                    const lat = parseFloat($map.dataset.lat) || 0;
                    const lng = parseFloat($map.dataset.lng) || 0;
                    const zoom = parseFloat($map.dataset.zoom) || 14;
                    const maxZoom = parseFloat($map.dataset.maxZoom) || 18;
                    const markers = parseJSON($map.dataset.markers, []);
        ...
        document.addEventListener("click", function(e) {
            const targetElement = e.target;
            if (targetElement.closest(".form-header__icon")) {
                document.documentElement.classList.add("search-open");
                e.preventDefault();
            } else if (!targetElement.closest(".form-header")) document.documentElement.classList.remove("search-open");
        });
        document.querySelectorAll(".building").forEach(building => {
            building.addEventListener("touchstart", function(e) {
                if (!this.classList.contains("active")) {
                    e.preventDefault();
                    document.querySelectorAll(".building.active").forEach(b => {
        ...
        window.addEventListener("load", () => {
            const el = document.querySelector(".js-typing");
            if (!el) return;
