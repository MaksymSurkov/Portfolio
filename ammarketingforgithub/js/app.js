(() => {
    ...
    const marquee = () => {
        ...
        $marqueeArray.forEach(($wrapper => {
            ...
            const breakpoint = parseFloat($wrapper.getAttribute("data-marquee-breakpoint"), 10);
            const hasBreakpoint = !Number.isNaN(breakpoint);
            let isActive = false;
            const shouldAnimate = () => {
                if (!hasBreakpoint) return true;
            ...
            const animation = () => {
                ...
                if (!head.querySelector(`style.${animName}`)) head.append($style);
            ...
            const init = () => {
                if (!shouldAnimate()) {
                    if (isActive) {
                        ...
                        if (children.length > initialElementsLength) children.slice(initialElementsLength).forEach((node => node.remove()));
    ...
    console.log;
    function initSolutionCards($wrapper, customOptions = {}) {
        const $cards = $wrapper.querySelectorAll(".item-solution__animate");
        ...
        $cards.forEach((($card, index0) => {
            ...
            const keyframes = {
                scale: [ opts.scaleFrom, opts.scaleTo ],
                opacity: [ opts.opacityFrom, opts.opacityTo ]
            };
            if (opts.rotateFrom !== opts.rotateTo) keyframes.rotateX = [ `${opts.rotateFrom}deg`, `${opts.rotateTo}deg` ];
            scroll_scroll(animate($card, keyframes), {
                target: $wrapper,
                offset: [ `${index0 / numCards * opts.offsetFactor}%`, `${index / numCards * opts.offsetFactor}%` ]
            });
        }));
    }
    window.addEventListener("DOMContentLoaded", (() => {
        ...
        const $cardsWrapper2 = document.querySelectorAll(".cards2");
        ...
        if ($cardsWrapper2.length && $cards2.length) $cardsWrapper2.forEach(($wrap => {
            $cards2.forEach(($title => {
                if (!$wrap.contains($title)) return;
                scroll_scroll(animate($title, {
                    scale: [ 1, .6 ],
                    opacity: [ 1, 0 ]
                }), {
                    target: $wrap,
        ...
        const solutionWrappers = document.querySelectorAll(".cards-solution__wrapper");
        if (solutionWrappers.length && typeof scroll_scroll === "function" && typeof animate === "function") solutionWrappers.forEach(($wrapper => {
            initSolutionCards($wrapper, {
                scaleTo: .6,
                opacityTo: 0,
                rotateTo: 10
            });
    ...
    function getBaseAngle(el) {
        const t = getComputedStyle(el).transform;
        if (!t || t === "none" || t.startsWith("matrix3d")) return 0;
    ...
    const script_observer = new IntersectionObserver((entries => {
        entries.forEach((entry => {
            const container = entry.target;
            const items = container.__rotateItems;
            if (!items) return;
            items.forEach((item => {
    ...
    document.querySelectorAll("[data-rotate]").forEach((el => {
        ...
        if (!container) return;
        const maxAttr = parseFloat(el.dataset.rotateMax);
        const max = Number.isNaN(maxAttr) ? 180 : maxAttr;
        const axis = (el.dataset.rotateAxis || "").toUpperCase();
        const item = {
            el,
            container,
            max,
        ...
        if (!container.__rotateItems) container.__rotateItems = [];
        container.__rotateItems.push(item);
        script_observer.observe(container);
    }));
    let ticking = false;
    function updateAll() {
        ticking = false;
        ...
        rotatingItems.forEach((item => {
            ...
            if (item.offset === null) {
                const cssAngle = getBaseAngle(item.el);
                item.offset = cssAngle - p * item.max;
            }
            const angle = item.offset + p * item.max;
            item.el.style.transform = `rotate${item.axis}(${angle}deg)`;
        }));
    }
    ...
    document.querySelectorAll(".wave-wrapp").forEach((block => {
        ...
        function animateWaves() {
            ...
            const baseX = isMobile ? 8 : 12;
            const baseY = isMobile ? 1.5 : 2.5;
            const phaseX = isMobile ? 2 : 3;
            waves.forEach(((wave, i) => {
                const phase = phaseOffset + i * .02;
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        const teamBlock = document.querySelector(".members-team");
        if (!teamBlock) return;
        console.log("no team block");
    ...
    document.querySelectorAll(".portfolio-block__item").forEach((item => {
        const imageWrapper = item.querySelector(".image-portfolio__wrapp");
        const imageItem = item.querySelector(".image-portfolio__item");
        if (!imageWrapper || !imageItem) return;
        item.addEventListener("mouseenter", (() => {
            const wrapperHeight = imageWrapper.offsetHeight;
            const itemHeight = imageItem.offsetHeight;
            const distance = itemHeight - wrapperHeight;
            if (distance <= 0) return;
            imageItem.style.transition = "none";
            imageItem.style.transform = "translateY(0)";
            void imageItem.offsetWidth;
            const duration = parseFloat(item.dataset.speed) || 4;
            imageItem.style.transition = `transform ${duration}s linear`;
            imageItem.style.transform = `translateY(-${distance}px)`;
        }));
        item.addEventListener("mouseleave", (() => {
            imageItem.style.transition = `transform 2s ease`;
            imageItem.style.transform = "translateY(0)";
        }));
    }));
    document.querySelectorAll(".klaviyo-screenshots__item").forEach((item => {
        const imageWrapper = item.querySelector(".image-klaviyo__wrapp");
        const imageItem = item.querySelector(".image-klaviyo__item");
        if (!imageWrapper || !imageItem) return;
        item.addEventListener("mouseenter", (() => {
            const wrapperHeight = imageWrapper.offsetHeight;
            const itemHeight = imageItem.offsetHeight;
